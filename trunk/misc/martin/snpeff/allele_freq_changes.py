...
1)	usage: python max_allele_change.py -i candidates.sync -b 100 -p 1,5,9+2,6,10+3,4,8 -s 1,2 -b 100 -t 0.0 -c 10 -m 500 -n 10 -o output
2)	This script needs the rpy2 package, which can be downloaded from here: http://sourceforge.net/projects/rpy/files/rpy2/ 
3)      this script calculates the allele frequency change of the rising allele summed over all replicates (i.e the sum of all counts for each allele within one timepoint). The results are printed instead of the cmh p-values
	""") 
...
parser.add_option("-r", "--compare", dest="compare", help="populations which should be compared")
parser.add_option("-a", "--names", dest="names", help="names of populations to be compred")
...
def combinations(inp,maxi,compare,hashi,code):
	a1=str(compare).split(",")
	b1=str(compare).split(",")
	for i in a1:
		for j in b1[1:]:
			if j!=i:
				hashi[code[i]+"_"+code[j]].append(freqhash(inp,int(j))[maxi]-freqhash(inp,int(i))[maxi])
		a1=a1[1:]	
		b1=b1[1:]
	return hashi
...
for l in comprep:	
	for i in l:
		allpops.append(i)
for l in options.compare.split(","):
	allpops.append(l)
for l in pops:
	comptime.append((l.split(",")[start],l.split(",")[end]))
c=0
out=open(options.out,"w")
frequencyhash=collections.defaultdict(lambda:[])
code=dict(zip(options.compare.split(","),options.names.split(",")))
for l in open(options.inp,"r"):
	...
	if SNP_test(l,allpops,options.maxcov,options.mincov,options.count)!=0:
		if max_allele_change(l,comptime)!="na":
			m=max_allele_change(l,comptime)
			combinations(l,m,options.compare,frequencyhash,code)
	if c%100000==0:
		print str(c)+" positions processed"

newf=zip(*frequencyhash.values())
out.write("\t".join(frequencyhash.keys())+"\n")
for l in newf:
