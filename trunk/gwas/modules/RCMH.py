...
class Utility:
	...
	@classmethod
	def getfishertest(cls,population1,population2):
		r=robjects.r
		tocomp=robjects.IntVector((population1[0],population2[0],population1[1],population2[1]))
		matr=r['matrix'](tocomp,nr=2)
		testres=r['fisher.test'](matr)
		pvalue=testres[0][0]
		assert(pvalue<=1)
		assert(pvalue>=0)
		return pvalue
	...
	def getCMHPvalue(cls,populations):
		...
		for i in range(1,len(populations),2):
			...
			response.append(base[0]) #basemajor
			response.append(derived[0]) #derivedmajor
