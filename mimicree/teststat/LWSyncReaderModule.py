...
class SyncReader:
	...
	def next(self):
		...
		pos=a.pop(0)
		refc=a.pop(0)
		majmin=SyncMajorMinorReader.__parse_sync(a)
		return (chr,pos,refc,majmin)

		
	@classmethod
	def parse_sync(self,entries):
		parsed=[]
		for e in entries:
			a=map(float,e.split(":"))
...
class CMHReader:
	...
	def next(self):
		...
		pos=a.pop(0)
		refc=a.pop(0)
		pvalue=a.pop() # remove the last one
		majmin=SyncReader.__parse_sync(a)
		return majmin
