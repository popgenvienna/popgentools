...
def getTargetSelected(raw):
	...
	for b in a:
		t=b.split("*")
		ts=TargetSelected(int(t[0]),t[1],t[2])
		toret.append(ts)
		targetsum=int(t[0])
	return (targetsum,toret)
	

# the interfering loci


parser = OptionParser()
parser.add_option("--input",dest="input",help="A file containing the dgrp haplotypes (MimicrEE input)")
parser.add_option("--selected",dest="selected",help="the selected snps in the form 30*0.1*0.5;30*0.5*0.5")
...
numsel,targetsel=getTargetSelected(options.selected)
...
for ts in targetsel:
	...
	# ignore all but the one actually used
for t in toprint:
	o=[]
	o.append(t.chr)
	o.append(t.pos)
