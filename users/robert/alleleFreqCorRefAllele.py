...
import collections
from  syncIOpg import SyncReaderRevAllele
import math
...
for chr,pos,temp,reff in SyncReaderRevAllele(options.input):
	for toco in toCompare:
		aindex=toco[0]
		bindex=toco[1]
		a=reff[aindex-1]
		b=reff[bindex-1]
		if(a[0]>=mincount and a[1]-a[0]>=mincount and b[1]-b[0]>=mincount and b[0]>=mincount and a[1]>=mincov and b[1]>=mincov):
			amf=float(a[0])/float(a[1])
			bmf=float(b[0])/float(b[1])
