...
# let's do it
perl $popte/identify-te-insertsites.pl --input $sam  --te-hierarchy-file $hier --te-hierarchy-level family --min-count 1 --min-map-qual 15 --narrow-range 100 --output $outfolder/tmp/tmp.tedirectional
# N-s
perl $popte/genomic-N-2gtf.pl --input $ref > $outfolder/tmp/polyn.gtf
# crosslink
perl $popte/crosslink-te-sites.pl --directional-insertions $outfolder/tmp/tmp.tedirectional --min-dist 85 --max-dist 400 --output $outfolder/tmp/tmp.teinsertions --poly-n $outfolder/tmp/polyn.gtf --te-hierarchy $hier
# estimate poly
perl ~r/popoolationte/estimate-polymorphism.pl --sam-file mapte/Dsim/Dsim.sort.sam --te-insert-file te/Dsim/raw/Dsim.teinsertions --te-hierarchy-file ~dmau/refg/Dsim/Dsim-combinedte-incM.hier --te-hierarchy-level family --min-map-qual 15 --output $outfolder/tmp/tmp.polytes
...
for $b in $bootstrap
do
 python $pgt/TE/TE/melsim/targetcoverage.py --tes $outfolder/results/${prefix}.mc10.euchr.polytes --target-cov $b --min-count > $outfolder/bootstrap/${prefix}.boot${b}.polytes
...
# statistics
perl $pgt/TE/mapstatdebug/debugstat-mapedreads-familywise.pl --input $sam --te-annotation $hier > $outfolder/stat/${prefix}.mapedreads.txt
perl $pgt/TE/mapstatdebug/debugstat-presFrag-familywise.pl --input $sam --te-annotation $hier > $outfolder/stat/${prefix}.presfrag.txt
