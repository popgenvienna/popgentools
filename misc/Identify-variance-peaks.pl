...
use List::Util qw/max min sum/;
    ...
    pod2usage(-msg=>"A step size has to be provided",-verbose=>1) unless $stepsize;
    ...
    my $hsslider=HighScoreSlider->new($input,$stepsize,$scorecalc);
    ...
    while(my $hsp=$hsslider->next())
    {
        ...
        my @vals=map {$_->{val}} @{$hsp->{val}};
        my $extreme=$pd_low ? min(@vals) : max(@vals);
        my $av=sum(@vals)/scalar(@vals);
        ...
        print $ofh "$hsp->{chr}\t$hsp->{start}\t$hsp->{end}\t$hs\t$av\t$extreme\n";
    }
...

{
    ...
    sub _nextWin
    {
        ...
        while($line=$self->_nextline)
        {
            ...
            $lastpos= $pos-$step if $lastpos eq "na";

            # check the abortion conditions;
            # 1..chromosome change
            # 2..position is not increment of the step size
            # 3..value equals to "na"
            if($curChr ne $e->{chr} or $pos-$lastpos !=$step)
            {
                ######################################################################################################
                $self->_bufferline($line);
                return $self->_annotateHighscore($curChr, $highscorestart, $highscoreend, $highscore, $highscorevals);
            }
            $lastpos=$pos;
            ...
            my $runningscore+=$s;
            ###########################################################################################################################
            return $self->_annotateHighscore($curChr, $highscorestart, $highscoreend, $highscore, $highscorevals) if ($runningscore<0);
    ...
    sub _parseVariance
    {
        my $self=shift;
        my $line=shift;
        my ($chr,$pos,undef,undef,$val)=split /\t/,$line;
        return {
            chr=>$chr,
            pos=>$pos,
            val=>$val
    ...
    sub _annotateHighscore
    {
        ...
        my $step=$self->{stepsize};
        my $interval=int($step/2);
        my $realstart=$start-$interval+1;
        my $realend=$end+$interval;
        
        #extract subhighscores
        my $entries=int(($end-$start)/$step)+1;
        my @vvals=@$vals;
